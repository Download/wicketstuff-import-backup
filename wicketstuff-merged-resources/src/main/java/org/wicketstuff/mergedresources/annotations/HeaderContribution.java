	...
	public HeaderContribution(Class<? extends Component> scope) {
		...
		for (String stylesheet : _stylesheets) {
			if (_stylesheetsMedia == null) {
				_headerContributors.add(HeaderContributor.forCss(scope, stylesheet));
			} else {
				_headerContributors.add(HeaderContributor.forCss(scope, stylesheet, _stylesheetsMedia));
			}
		}
		
		for(String script : _scripts) {
			_headerContributors.add(HeaderContributor.forJavaScript(scope, script));
		}
	}
